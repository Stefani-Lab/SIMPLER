...
Par_1 = [Center+centroid , sqrt(Center*Center'+Mz+2*xnew)];

% Compute diferences from fitted radius and actual radius for each point
 
dist_centroid = zeros(size(XY,1),1);

for i=1:size(XY,1);
    dist_centroid(i,1) = 100*sqrt((Par_1(3)-pdist([XY(i,:);Par_1(1) Par_1(2)]))^2)/Par_1(3);
end
percent_excluded = 25;


Par = [Par_1 mean([prctile(dist_centroid,percent_excluded) prctile(dist_centroid,100-percent_excluded)])];
